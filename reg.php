...
<form name="regrom" method="post" action="<? $_SERVER['PHP_SELF'] ?>" >
    <table class='outer-border' id='main'>	
    <tr>
    <td class="textbox"><? echo $locate ['409'] ?>*</td>
    <td class="table-body"><input name="ma" type="text" /></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['410'] ?>*</td>
    <td class="table-body"><input name="p" type="password" id="password" /></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['445'] ?>*</td>
    <td class="table-body"><input name="pp" type="password"/></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['411'] ?>*</td>
<?php 
echo '<td class="table-body"><select name="plz">';
echo '<option value="0">Bitte w&auml;hlen</option>';
$stmt = $mysqli->prepare("SELECT id, zipcode FROM ".ZIP." ORDER BY ".ZIP." ASC");
$stmt->execute();
$stmt->bind_result($id, $zip);
while ($stmt->fetch()) {
...
?>
    ...
    <td class="table-body"><input name="email" type="email" ></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['406'] ?>*</td>
    <td class="table-body"><input name="na" type="text" maxlength="255"></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['407'] ?>*</td>
    <td class="table-body"><input name="vna" type="text" maxlength="255"></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['415'] ?>**</td>
    <td class="table-body"><input name="tel" type="tel" maxlength="255"></td>
    </tr>
    <tr>
    ...
    <tr>
    <td class="textbox"><? echo $locate ['418'] ?> Möchtest du Anonym bleiben, dann machen wir das für dich ;)</td>
    <td class="table-body"><input name="ano" type="checkbox" value="1" /></td>
    </tr>
    <tr>
    <td class="textbox"><? echo $locate ['408'] ?>*</td>
    <td class="table-body"><input name="str" type="text" /></td>
        <input name="okr" type="hidden" value="okr" />
    </tr>
        <tr>
        <td class="table-body"><?php echo file_get_contents("http://www.kostenloses-captcha.de/image.php?ref=".$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']."&rem=".$_SERVER['REMOTE_ADDR']."&format=0&bg=18&csscolor=30addc&cssborder=0e4357&cssfont=000000"); ?></td></tr>
    <tr>
    <td class="table-body"><input class="argbut" name="okr" type="button" value="Speichern" onclick="if (chkregrom() == true){formhash(this.form, this.form.password); }" /></td></tr>
    </table>
    </form>
<?
if ($_REQUEST['okr']) {
